<template>
  <div class="rounded-full">
    <svg class="rounded-full" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 53.48 53.48">
      <defs></defs>
      <g id="LinkedinLayer_2" data-name="LinkedinLayer 2">
        <g id="LinkedinLayer_1" data-name="LinkedinLayer 1">
          <circle id="Ellipse_86-2" data-name="Ellipse 86-2" class="linkedin-1" cx="26.74" cy="26.74" r="26.74" />
          <path id="Path_2050" data-name="Path 2050" class="linkedin-2"
            d="M42.72,28.89v11H36.33V29.63c0-2.59-.93-4.35-3.24-4.35a3.5,3.5,0,0,0-3.28,2.34,4.33,4.33,0,0,0-.21,1.56V39.91H23.21s.08-17.42,0-19.22H29.6v2.73l0,.07h0v-.07a6.33,6.33,0,0,1,5.76-3.17c4.2,0,7.36,2.75,7.36,8.65ZM16.52,11.42a3.33,3.33,0,1,0-.43,6.65h.39A3.33,3.33,0,0,0,17,11.43h-.5ZM13.29,39.91h6.39V20.69H13.29Z" />
        </g>
      </g>
    </svg>
  </div>
</template>
<style lang="scss" scoped>
@import "~/assets/scss/icon/linkedin.scss";
</style>
